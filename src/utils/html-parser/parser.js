...
function readAttribute(context) {
  ...
  if (context.current === '=' || context.peekIgnoreWhitespace() === '=') {
    ...
    const match = attributeValueRegex.exec(context.substring) || [0, ''];
    value = match[1];
...
function readAttributes(context, isXml) {
  ...
  while (!context.isEof() && !isClosingToken()) {
    handled = false;
    if (context.current === '<') {
      for (const callbackName in context.regex.dataElements) {
        if (!Object.prototype.hasOwnProperty.call(context.regex.dataElements, callbackName)) {
          continue;
        }

        const dataElement = context.regex.dataElements[callbackName];
        const { start } = dataElement;
        let isValid = false;
...
function parseOpenElement(context) {
  const name = context.readRegex(context.regex.name);
  ...
  const regex = new RegExp(`^([\\s\\S]*?)(?:$|</(${name})\\s*>)`, 'i');
  const match = regex.exec(context.substring);
  context.read(match[0].length);
  if (match[1]) {
    ...
    case 'undefined': {
      ...
      switch (typeof end) {
        case 'string':
          ...
          break;
        case 'object': {
          let match = context.substring.match(end);
          if (match) {
...
function parseNext(context) {
  if (context.current === '<') {
    const next = context.substring.charAt(1);
 ...
 * @property {String|RegExp|Function} start - start of data element,
 * for example '<%' or /^<\?=/ or function(string){return string.slice(0, 2) === '<%' ? 2 : -1;}
 * @property {RegExp|Function} data - content of data element,
 * for example /^[^\s]+/ or function(string){return string.match(/^[^\s]+/)[0];}
 * @property {String|RegExp|Function} end - end of data element,
 * for example '%>' or /^\?>/ or function(string){return 2;}
 */
...
exports.sanitize = function (htmlString, removalCallbacks) {
  ...
  const callbacks = {
    ...
    closeOpenedElement(name, token) {
      name = name.toLowerCase();
      if (token.length === 2) {
        // self closing
        const scope = tagStack.pop();
        if (scope === last(ignoreStack)) {
          ignoreStack.pop();
        }
      }
